...
object Day13 {
   ...
   */
  private def solve2(lines: List[String]): Long = {
    val departures = lines.last.split(",").zipWithIndex.filter(_._1 != "x").map(t => (t._1.toLong, t._2.toLong))
    val res = departures.tail
      .foldLeft((departures.head._1, 0.toLong))((acc, el) => (acc._1 * el._1, findNextValue(acc._1, acc._2, el)))
    res._2
  }
